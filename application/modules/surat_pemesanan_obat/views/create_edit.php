...
<?= page_header("Surat Pemesanan Obat"); ?>
...
<section class="content">
  <div class="box">
    ...
    <?php echo form_open($form_action,  array('id' => 'form', 'class' => 'form-horizontal')); ?>
      <div class="box-body">
        <?= input_select(['label' => 'Jenis', 'name' => 'jenis_select', 'list' => $jenis, 'selected_value' => $orders['type_order'] ?? $type_order], true); ?>
        ...
        <?= input_text(['label' => 'Jabatan', 'name' => 'position', 'value' => $orders['position'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'No. SIPA', 'name' => 'sipa', 'value' => $orders['sipa'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'No. SIA', 'name' => 'sia', 'value' => $orders['sia'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'Nama PBF', 'name' => 'pbf_name', 'value' => $orders['pbf_name'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'Alamat PBF', 'name' => 'pbf_address', 'value' => $orders['pbf_address'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'Telepon PBF', 'name' => 'pbf_phone', 'value' => $orders['pbf_phone'] ?? '', 'disabled' => (isset($orders)) ? true : false], true); ?>
        <?= input_text(['label' => 'Tanggal', 'name' => 'order_date', 'value' => (isset($orders)) ? date('d/m/Y', strtotime($orders['order_date'])) : date('d/m/Y'), 'class' => 'datepicker disable-autocomplete', 'disabled' => (isset($orders)) ? true : false], true); ?>
              ...
              <?php if (isset($order_detail) && count($order_detail) > 0) { ?>
                <?php foreach($order_detail as $key => $pd) { ?>
                  <tr id="empty_data">
                    <td><?= ++$key; ?></td>
                    <td><?= $pd['name']; ?></td>
                    <td><?= $pd['active_substance']; ?></td>
                    <td><?= $pd['preparation']; ?></td>
                    <td><?= $pd['quantity']; ?></td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="form_add_barang" class="d-none" autocomplete="off">
          ...
          <?= input_text(['label' => 'Zat aktif obat tertentu', 'name' => 'active_substance']); ?>
          <?= input_text(['label' => 'Sediaan', 'name' => 'preparation']); ?>
          <?= input_text(['label' => 'Jumlah', 'name' => 'quantity', 'type' => 'number']); ?>
...
<script>
  ...
  $('#save_button').click(function(e) {
    ...
    var zatAktif = $('#active_substance').val() || ''
    var sediaan = $('#preparation').val() || 0
    ...
    var inputProduct = `<input type="hidden" name="products[]" value="${productId}|${zatAktif}|${sediaan}|${quantity}|${keterangan}">`
