...
<section class="content">
  <div class="box">
    <div class="box-header with-border" style="display: flex;justify-content: flex-end;">
      <div>
        <button id="add_pelanggan" class="btn btn-success"><i class="fa fa-plus margin-right-lg"></i> Tambah</button>
      </div>
    </div>
    <div class="box-body">
...
<!-- Modal Pelanggan-->
<div class="modal fade" id="modal_master_pelanggan">
  <?= form_open('master_pelanggan/store', 'id="form_master_pelanggan"'); ?>
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <?= input_text(array('name' => 'nama', 'label' => 'Nama Pelanggan *'));?>
...
<script>
  ...
  var argDataTables = {
    url: "<?= base_url(); ?>/master_pelanggan/get_datatables_json",
    columns: [                        
      {"data": "name"},
      {"data": "name"},
      {"data": "address"},
  ...
  $('#table').on("click", ".item-edit", function(e) {
    ...
    $.ajax({
      ...
      $('#form_master_pelanggan').attr('action', formUrl)
      $('#nama').val(res.name);
      $('#alamat').val(res.address);
      $('#nomor_kontak').val(res.contact);

       $('#modal_master_pelanggan').modal('show');
    });
  });
  ...
  $('#add_pelanggan').click(function() {
    $('#form_master_pelanggan')[0].reset();
    var url = $('#form_master_pelanggan').attr('action')
  ...
  $('#form_master_pelanggan').submit(function(e) {
      e.preventDefault()
        ...
        success: function(data) {
          if (data.success) {
            show_sweet_alert({
              ...
              dataTable.draw()
              $('#modal_master_pelanggan').modal('hide')
            })
          } else {
  ...
  $('#modal_master_pelanggan').on('hidden.bs.modal', function() {
    clearInputText('nama')
  })
</script>